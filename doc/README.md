...
> _It is designed to be fast and lean and incorporates modern features based on open standards._
...
This image is built with a strict focus on security, correctness, and predictable performance. Every design choice is intentional and avoids unnecessary components, dynamic behavior, or runtime variability. The goal is to provide a resolver that behaves consistently across architectures, is resistant to supply chain risks, and performs well under real world load without requiring external tuning.

The build pipeline uses a controlled multi stage process that isolates the OpenSSL build environment from the Unbound compilation. This ensures that cryptographic primitives, QUIC support, and TLS behavior are identical across all supported platforms. The OpenSSL toolchain is compiled with architecture specific optimizations, which improves handshake performance, reduces CPU usage, and avoids the inconsistencies that occur when relying on distribution provided libraries.

All source code and trust anchors are verified before use. Unbound is downloaded directly from NLnet Labs and validated using SHA256 checksums and PGP signatures. Root hints and the root zone are also validated using MD5 checksums and PGP signatures. This provides a verifiable trust chain from the resolver to the DNS root and protects against tampering at every stage of the build. The resolver starts with a known good trust anchor and a validated view of the DNS root, which is essential for DNSSEC correctness.

The runtime environment is intentionally minimal. It contains only the binaries and libraries required for Unbound to operate. There is no package manager, no shell beyond what is required for the entrypoint, and no dynamic components that could introduce unexpected behavior. The container runs as an unprivileged user by default and supports both unprivileged and privileged port binding. This reduces the attack surface and prevents privilege escalation paths that are common in larger images.

Performance is improved by using a distroless scratch base. The resolver starts quickly, memory usage is predictable, and there are no background processes competing for resources. Libevent, ngtcp2, nghttp2, and hiredis are included to support high performance event handling, QUIC, HTTP based transports, and Redis backed caching. These components are compiled with the same toolchain as Unbound to ensure ABI compatibility and optimal performance.

The image supports DNSSEC, DNSCrypt, DNSTap, DNS64, DNS over TLS, DNS over HTTPS, and DNS over QUIC. QUIC support is provided through a dedicated build variant that uses the QUIC capable OpenSSL toolchain. Redis integration is available through both UNIX sockets and network connections, which allows low latency caching in high throughput environments. EDNS Client Subnet is supported for deployments that require it. Optional healthchecks and Zabbix compatible statistics export are available without relying on third party tools.
